...
**out-of-scope.** Ask whether a rejected enhancement is recorded in a `.out-of-scope/` file as well as closed: `out-of-scope: on`. Default **off for GitHub and Linear**, **on for local-markdown**. Write `out-of-scope: on` under `## Issue tracker` only when the user wants the non-default. When on, `/triage`'s `wontfix` writes the record; when off, the reasoning lands in the closing comment alone.
