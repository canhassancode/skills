---
name: bootstrap
description: One-time per-repo setup for the engineering skills — configures the issue tracker, triage-label vocabulary, and domain-doc layout for BOTH the triage-graph lane (triage/to-proposal) and the planning lane (to-spec/to-tickets/wayfinder/code-review/implement). Use when onboarding a repo to these skills or switching its tracker.
...
- **Triage-graph lane** (`triage`, `to-proposal`) reads a `tracker:` preference from the repo's `CLAUDE.md` and loads a global adapter (`~/.claude/skills/triage/issue-tracker-<tracker>.md`). GitHub and Linear only.
...
- **GitLab / local / other** — no triage-graph adapter exists. Set the `tracker:` line for the record, and **tell the user the triage-graph lane (`triage`/`to-proposal`) is GitHub/Linear-only** — only the planning lane is wired for this tracker.
...
- `docs/agents/triage-labels.md` — from [triage-labels.md](./triage-labels.md). Six canonical roles across two lanes: inbound (`needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`) and planning (`needs-alignment`). (No `prd` label — the planning lane holds a ticket in `needs-alignment` until stage 2 publishes the contract.)
