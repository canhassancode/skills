...
On every RED→GREEN loop, `/commit` — but only on a real green: tests exiting 0 is the gate, not your say-so. Each commit pushes the state out to git, the ticket and the tests, so nothing left in the window is worth carrying forward. Clearing the window is not this agent's move — that is the operator's `/clear` between runs, or a fresh implementer dispatched per slice; within a single run the lever is the lean-window discipline above, not a reset.
