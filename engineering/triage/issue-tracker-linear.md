...
The AI disclaimer defaults to **ON** for Linear (shared team space where AI authorship signal has value for teammates). Every triage comment and issue body starts with `> *This was generated by AI during triage.*` unless the repo has set `triage-disclaimer: off` via `setup-tracker`.
