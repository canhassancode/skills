...
An agent brief is a structured comment posted on a tracker issue when it moves to `ready-for-agent`. It is the authoritative specification that an AFK agent will work from. The original issue body and discussion are context — the agent brief is the contract.
...
- **Good:** "Querying the tracker for `needs-triage` issues returns items that have been through initial classification"
