...
The AI disclaimer defaults to **OFF** for GitHub (personal tracker, where AI authorship is self-evident noise). The repo can override this via `setup-tracker`'s `triage-disclaimer: on` preference. When on, every triage comment and issue body starts with `> *This was generated by AI during triage.*`.
