...
## Status block and Updates section (GitHub only)
...
On every state transition on GitHub, `/triage` does three things:
...
6. **Maintain the Status block and Updates section (GitHub only)** per the [Status block pattern](#status-block-and-updates-section-github-only). After any outcome that doesn't close the issue, edit the body's `## Status` block to update the date and link to the new comment. For PRDs, prompt the maintainer about an `## Updates` entry. On **Linear**, edit the description in place via `save_issue` — the adapter handles it, and comments carry only the delta and reasoning.
