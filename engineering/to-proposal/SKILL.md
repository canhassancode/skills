...
2. **Publish to Notion on confirm** — render a cleaned proposal to the Notion proposals space via the Notion MCP. Crossing to Notion is an **audience pass**, not a confidentiality scrub: drop personal candour ("Pete's estimate is fantasy" → "the 2-week estimate looks optimistic given X"), drop incidental paths/SHAs, and re-attach real customer identity here (Notion is the authorised surface where colleagues are entitled to it) from live context — never carried from the vault, where it's pseudonymised. Class A was never in the draft to strip.

If `~/Obsidian/` is absent, draft inline and skip step 1.

## Process

1. **Identify the audience and the ask.** Who decides — Pete, a stakeholder, a VP? How hard you argue and how much context you front-load depends on it. State the audience at the top of the draft. If unclear, ask the one question.
...
- `/to-prd` — the sibling for build specs aimed at implementers (GitHub, not Notion).
