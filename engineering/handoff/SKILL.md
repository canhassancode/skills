...
- **If `~/Obsidian/` exists** — write to `~/Obsidian/Handoffs/<YYYY-MM-DD-HHMM>-handoff.md` with frontmatter `type: handoff` + `created:`. This joins the vault graph, so **wikilink the Library/Profile pages the next session will need** (`[[Search Resolver V2 (Carpata)]]`, `[[Carpata]]`, …) — a quick `/ask` finds them. Conform to `~/Obsidian/CONVENTIONS.md`. The handoff is **Class B working context** — never write Class A (secrets, PII, file paths + SHAs, verbatim proprietary source).
