...
Resumes a **handoff** written by `/handoff`. The session-continuity counterpart to `/pickup` (which verifies a *ticket*) — kept distinct so neither term is overloaded. Read `~/Obsidian/CONTEXT.md` and `~/Obsidian/CONVENTIONS.md` first.
...
1. **Find** the handoff — the most recent file in `~/Obsidian/Handoffs/` (or the one named by the user). If `Handoffs/` is empty, say so and stop.
