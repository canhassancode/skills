...
Keep it to one scannable line. Link the project hub (`[[Acme]]`, `[[Brushfeed]]`) so the hub's backlinks become a timeline. Drop the `[blocked: …]` clause unless there's a blocker; drop the hub if there's no obvious one.
...
- 09:12 — Standup: replanned the week around the dashboard work. Carried the QA triage to a colleague. · [[Acme]]
- 11:40 — Grill on the widget refund flow → routed to a Notion proposal. Decided a scoped session token. [blocked: scope, on the decision-maker] · [[Acme]]
