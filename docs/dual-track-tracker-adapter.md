...
4. **Reel back exactly one thing from Pocock:** his tracker-agnostic `setup` pattern (asks GitHub|Linear|local, writes the per-repo config). Everything else that drifted — the Obsidian second brain, the log spine, `propose`/`pickup`/`receive` — is *ahead* of upstream, not behind. Reeling those back would be regression.
