...
- **`needs-alignment` is a real intermediate state, and the by-construction guarantee ends.** A ticket born from `/align` exists on the board before it is buildable, so the label is the guardrail that stops it being taken: it means **the body is not yet the contract**. The body stays the alignment's own record of intent; stage 2 reads it and proposes what to cut from it — the tickets and the stack — rather than rewriting it. `/align` does not create that ticket on entry — it creates one only on the operator's word — so a first pass that closes **dropped**, or that is still foggy, leaves no tracker node behind.
- **A pass closes with a verdict, not a feeling.** Aligned (gate met, unresolved at zero → stage 2), fog (every unresolved item carries a route → another pass, and the routes are the plan), dropped (the work is not worth doing → nothing new is created, and a ticket that exists moves to `wontfix`), or thin (unresolved without routes, axes unmarked, no decisions recorded → the failure). Fog and thin look identical from outside; the route is the discriminator. A dropped pass records its reason, so the decision is not re-opened.
- **The artefact is the deliverable, and it is plain text, published into the ticket.** The **body** carries the state — scenarios and their outcomes, the settled interfaces, the decision table with its rejected alternatives, the axes, and an unresolved list that counts to zero. Each pass's **comment** carries what that pass produced: its verdict and delta, the diagrams, the sketches as they stood, and the reasoning. All of it renders on the trackers, and the alignment record never lands on a branch.
...
- **`gh stack` carries the change.** The base layer is `docs/` — the ADR, `CONTEXT.md`, research, and only those: a branch is cut when the work changes one of those documents, and the alignment record itself never lands on one. Layers above carry work. Branch names are semantic: `docs/`, `feat/`, `fix/`.
