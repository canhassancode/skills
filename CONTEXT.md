...
- The planning lane is three acts: `/align` → `/cut` → `/build`. `/implement` and `/code-review` retire into **Build** and **Review** once stage 3 has run in anger; until then the inbound lane keeps `/implement`, and `/review` keeps `/code-review` as its engine.
